...
def evaluate_results_with_andreff(my_solver, T_v_rel, T_c_rel, scale, cons):
    ...
    dual_time, dual_gt_value, dual_primal, dual_gap, dual_opt, dual_solution, dual_flag = my_solver.dual_solve(cons, verbose=False)
    # rel_time, rel_gt_value, rel_primal, rel_gap, relax_opt, relax_solution, rel_flag = my_solver.relax_solve(cons, verbose=False)
    rel_flag = False
...
def main(rotation_error, trans_error):
    ...
    if not brookshire:
        ...
        scale = .5 # 2.
        data_filename = "./results/matt_N_{:}_per{}t{}r_".format(num_cases, int(trans_sigma * 1000), int(rot_sigma * 1000)) + data_filename
    else:
        # Name the relevant filenames
...
if __name__ == "__main__":
    # R = [0.01, 0.015, 0.01, 0.015]
    # T = [0.015, 0.01, 0.005, 0.005]

    # R = [0.01, 0.02, 0.01, 0.02, 0.01, 0.02]
    # T = [0.005, 0.005, 0.01, 0.01, 0.015, 0.015]

    R = [0.01, 0.05, 0.01, 0.05, 0.01, 0.05]
    T = [0.01, 0.01, 0.025, 0.025, 0.05, 0.05]

    for i in range(min(len(R), len(T))):

        print('i = {:}'.format(i))
        results = main(R[i], T[i])
        results = results['RCH']

        # Print results
        print("[sigma_r, sigma_t] = [%g, %g]" % (R[i], T[i]))
